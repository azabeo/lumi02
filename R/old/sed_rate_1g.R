...
#' sed_rate_conc()
#' @export
sed_rate_1g <- function(dat,expr,par.start,consts,mg,predictor="time",response="trans",rpm_col="rpm"){
  
  absorb <- absorbance(dat,response)
  conc <- concentration(absorb,mg)
  
  data <- fitEachRpm(conc,expr,par.start,consts,mg,predictor,rpm_col=rpm_col)
...
#' sed_rate_dataframe()
#' @export
fitEachRpm <- function(dat,expr,par.start,consts,mg,predictor="time",response="conc",rpm_col="rpm"){
  # create a list of dataset for applying derive
  rpms = unique(dat[rpm_col])
  i=1
